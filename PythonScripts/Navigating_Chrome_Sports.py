...
def firstWebsiteSportHTMLCollector(link:str):
    try:
        ...
        while matchUpLinks == "undefined":
            findImageOnScreen("websiteImages/firstWebsiteLoadIconSports.png",timeoutDuration = 20,grayscaleFlag=True) #This website needs some extra time to load. This HTML icon indicates that all the HTML has been loaded and can be extracted.
...
def secondWebsiteSportHTMLCollector(link:str):
    try:
        htmlFetchingErrorWebhook = SyncWebhook.from_url(os.getenv('HTML_FETCHING_ERROR_NOTIF'))
        if link =="a":
            return
        ...
        for index in range(len(matchUpLinks)):
            ...
            try: #I have an inner try and catch loop here because if we can't find the oddsLoadedIcon on just one of the pages, I don't want the program to skip all the other pages because the icon might be there. If I just had one try and catch in the collector function, then I would skip all the odds pages as soon as I can't find the icon.
                matchWinnerIconLocation=findImageOnScreen("websiteImages/secondWebsiteOddsLoadedSportsIcon.png",timeoutDuration = 25,grayscaleFlag=True,confidenceValue=0.99)
            except ImageNotFoundOnScreenError as inst:
                htmlFetchingErrorWebhook.send(content=str(inst)+"==========================================================================")
                continue
            ...
            matchWinnerIconLocation=findImageOnScreen("websiteImages/secondWebsiteOddsLoadedSportsIcon.png",timeoutDuration = 20,grayscaleFlag=True,confidenceValue=0.99,regionBox=(556,606,693,624)) 
...
def thirdWebsiteSportHTMLCollector(link:str):
    try:
        htmlFetchingErrorWebhook = SyncWebhook.from_url(os.getenv('HTML_FETCHING_ERROR_NOTIF'))
        if link =="a":
            ...
            return
        goToWebsite(link)
        findImageOnScreen("websiteImages/thirdWebsiteSportsLoadIcon.png",timeoutDuration = 20,grayscaleFlag=True)
...
def fourthWebsiteSportHTMLCollector(link:str):
    htmlFetchingErrorWebhook = SyncWebhook.from_url(os.getenv('HTML_FETCHING_ERROR_NOTIF'))
    if link =="a":
    ...
    try:
        ...
        p.start()
        findImageOnScreen("websiteImages/fourthWebsiteSportLoadIcon.png", timeoutDuration = 20, confidenceValue=0.90, grayscaleFlag=True,regionBox=(445,345,400,165))
        ...
        htmlFetchingErrorWebhook.send(content=str(inst)+"==========================================================================")
        closeFirefox()
        if inst.imageFile == "websiteImages/fourthWebsiteSportLoadIcon.png":
            htmlFetchingErrorWebhook.send("It also might be possible that the link for this website's event is invalid so double check the environment file.\n\n==========================================================================")
    except NoAnchorTagsPresentInLocalHTMLFileError as inst:
        htmlFetchingErrorWebhook.send(content=str(inst)+"==========================================================================")
